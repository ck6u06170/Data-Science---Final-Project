...
class trainGenerator(Dataset):
    ...
    def load_image(self, index,norm=True,no_transpose=False):
        ...
        # ~ print('data img = ',img.shape,type(img))
    ...
    def load_HE(self, index,norm=True,no_transpose=False):
        ...
        HE = cv2.resize(HE, (512, 256), interpolation=cv2.INTER_CUBIC)
...
def	soft_dice(input, target):
    ...
    #print('dim = ',dim)
    #print('torch.sum(intersection,dim) + smooth = ',torch.sum(intersection,dim) + smooth)
    #print('torch.sum(input_flat*input_flat,dim = ',torch.sum(input_flat*input_flat,dim))
    #print('torch.sum(target_flat*target_flat,dim) + smooth = ',(torch.sum(target_flat*target_flat,dim) + smooth))
    loss = 2. * (torch.sum(intersection,dim) + smooth) / (torch.sum(input_flat*input_flat,dim)+ torch.sum(target_flat*target_flat,dim) + smooth)
