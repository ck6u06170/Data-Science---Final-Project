...
class RandomSampleCrop(object):
    ...
    def __call__(self, img, mask,HE,edge):
        ...
        while True:
            # ~ print(aqqs)
            # ~ print('self.min_win*width = ',self.min_win*width)
            # ~ print('width = ',width)
            # ~ print('self.min_win = ',self.min_win)
            ...
            # ~ print('current_mask = ',current_mask.shape)
            # ~ print('current_HE = ',current_HE.shape)
            # ~ print('current_edge = ',current_edge.shape)
            ...
            HE      = cv2.resize(HE, (height, width), interpolation=cv2.INTER_CUBIC)
            edge    = cv2.resize(edge, (height, width), interpolation=cv2.INTER_CUBIC)
            # ~ print('H')
            # ~ print('img = ',img.shape)
            ...
            return img ,mask ,HE ,edge
            # ~ return current_img ,current_mask ,current_HE ,current_edge
