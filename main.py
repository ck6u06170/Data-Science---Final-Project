...
class test_model(object):
    ...
    def test(self):
        ...
        for index in range(len(self.datagen)):
            rtime_print('{}/{}'.format(index,len(self.datagen)))
    ...
    def predition(self,img,HE,file):
        with torch.no_grad(): 
            ...
            out_pre = np.array((nuclei>self.conf.cutoff)*255,dtype='uint8')
            out_pre = cv2.resize(out_pre, (1716, 942), interpolation=cv2.INTER_CUBIC)
            io.imsave(os.path.join(self.conf.save_path,file[0:(len(file)-4)]+'-nuclei.png'),out_pre)
...
class train(object):
    def __init__(self,conf):
        ...
        self.net       = torch.nn.DataParallel(model.net(), device_ids=devicess).cuda()
        # ~ model = torch.nn.DataParallel(model.net(), device_ids=devicess)
        ...
        data_transform =  transform.Compose([
            transform.RandomSampleCrop(0.4),#not work , use resize to 256,256
    ...
    def epoch_train(self,epoch):
        ...
        for d in self.train_loader:
            ...
            with torch.enable_grad():   
                ...
                print(type(contourRGB))
                #nuclei=torch.squeeze(nuclei.cpu())
                print(type(contourRGB))
                contourRGB = contourRGB.cpu().detach().numpy()
                print(type(contourRGB))
                #nuclei = np.array((nuclei>0.5)*255,dtype='uint8')
                print(contourRGB.shape)
                contourH=torch.squeeze(contourH.cpu())
                contourRGB=torch.squeeze(contourRGB.cpu())
                io.imsave(os.path.join(self.conf.save_path,'0-nuclei.png'),nuclei*255)
                io.imsave(os.path.join(self.conf.save_path,'0-RGB.png'),np.array(contourRGB.detach()*255,dtype='uint8'))                      
                io.imsave(os.path.join(self.conf.save_path,'0-H.png'),np.array(contourH.detach()*255,dtype='uint8'))
                #print('nuclei = ',nuclei.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch',default=96,required=False,type=int,
					help='num of epoch')
